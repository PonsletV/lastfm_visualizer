...
# add artist + album in data
lastFM2$album_artist <- paste(lastFM2$album, lastFM2$artist, sep=" - ")
...
lastFMGrouped <- group_by(lastFM2, album_artist, monthID, date) %>%
  summarise(count=n())
...
allBands <- unique(lastFMGrouped$album_artist)
allMonthIDs <- unique(lastFMGrouped$monthID)
allDates <- unique(lastFMGrouped$date)

refDF <- data.frame(album_artist = rep(allBands, length(allMonthIDs)),
...
full_agg <- full_agg[, c("album_artist", "monthID", "date", "count")]
...
full_agg <- full_agg %>% group_by(album_artist) %>%
  mutate(cum_count=cumsum(count))

full_agg <- full_agg[full_agg$cum_count != 0,]

full_agg$value <- full_agg$cum_count
full_agg$category <- full_agg$album_artist
# other solution
